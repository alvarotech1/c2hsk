...
import Control.Monad.State   (StateT, get, put, gets, modify, runStateT)
...
declareVar v t = do
  ...
  modify $ \e ->
    ...
    in e { scopes   = M.insert v ref top : rest
...
evalComm (FuncDef retT "main" _ body) ind = do
  ...
  retRef <- freshTmp
  emit (indentStr (ind+2) ++ retRef ++
        " <- newIORef (Nothing :: Maybe " ++ translateType retT ++ ")")
...
evalComm (FuncDef retT name params body) ind
  | name /= "main" = do
      ...
      let paramTys = map (translateType . fst) params
          ...
          typeSig  = name ++ " :: " ++ concatMap (++ " -> ") paramTys
                               ++ "IO " ++ retTyStr
      emit ""
      emit typeSig
      ...
      case retT of
        ...
        _     -> do
          ...
          emit (indentStr (ind+4) ++ "Nothing -> error \"\\\"" ++ name
...
evalComm (LetType (TArray t size) v (InitList xs)) ind = do
  ...
  emit (indentStr ind ++ tmpArr
        ++ " <- (A.newListArray ((0 :: Int32)," ++ show (size - 1) ++ " :: Int32) "
        ++ listTok
        ++ " :: IO (A.IOArray Int32 " ++ translateType t ++ "))")
...
evalComm (AssignArr arrVar idxExp rhsExp) ind = do
  ...
  emit (indentStr ind ++ "A.writeArray " ++ tmpArr
        ++ " (" ++ idxTok ++ ")"
        ++ " (" ++ rhsTok ++ ")")
  continue_
...
evalComm (Return expr) ind = do
  ...
  case mSlot of
    Nothing   ->                       -- impos-tile (return fuera de función)
    ...
    Just ref -> do
      ...
      emit (indentStr (ind+2) ++ "Nothing -> writeIORef "
                       ++ ref ++ " (Just (" ++ valTok ++ "))")
...
evalComm (Printf fmt args) ind = do
  argChunks <- mapM (\a -> do
                        ...
                        pure ([], tok)) args      -- pres vacíos porque los emitió evalExp
...
evalComm (While cond body) ind = do
  ...
  emit (indentStr inner ++
        "when (not quit && not done && (" ++ cTok ++ ")) $ do")
...
evalComm (For mInit cond mStep body) ind = do
  ...
  emit (indentStr inner ++ "quit <- readIORef " ++ brkRef)
  case mRet of
    Just r -> emit (indentStr inner ++
                    "done <- fmap isJust (readIORef " ++ r ++ ")")
    Nothing-> emit (indentStr inner ++ "let done = False")
...
evalExp (ArrayAccess arrVar idxExp) ind = do
  ...
  valTmp <- freshTmp
  emit (indentStr ind ++ tmpArr ++ " <- readIORef " ++ refTok)
  emit (indentStr ind ++ valTmp ++ " <- A.readArray "
        ++ tmpArr ++ " " ++ idxTok)
  pure valTmp
...
evalExp (InitList xs) ind = do
  ...
  emit (indentStr ind ++ tmpV
        ++ " <- newIORef =<< A.newListArray (0," ++ show (len-1) ++ ") " ++ body)
  pure tmpV
...
evalExp (AddrOf _) _ = error "AddrOf todavía no soportado (pendiente)"
evalExp (Deref  _) _ = error "Deref  todavía no soportado (pendiente)"
...
mutateVar :: Variable               -- ^ variable a mutar
          -> String                 -- ^ operador ("+ 1"  o  "- 1")
          -> Bool                   -- ^ True → pre (devuelve nuevo); False → post (devuelve viejo)
          -> Int                    -- ^ indent
          -> Gen String
...
translatePrintf
  ...
  -> [([String], String)]       -- ^ (pres, token) por argumento
  -> ([String], String)         -- ^ (pres totales, línea final)
...
genNewArray :: Type      -- ^ tipo base del array
            -> Int       -- ^ tamaño
            -> String    -- ^ token del valor inicial (puede llevar :: …)
            -> Int       -- ^ indent
            -> Gen String
genNewArray t size initTok ind = do
  tmpArr <- freshTmp
  emit $ indentStr ind ++ tmpArr
      ++ " <- (A.newArray (0," ++ show (size - 1) ++ ") ("
      ++ initTok ++ ") :: IO (A.IOArray Int32 " ++ translateType t ++ "))"
  pure tmpArr
